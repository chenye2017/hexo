...
```
怎么会导致跨域，为什么会出现跨域
schema，或者ip地址，或者端口号 不同都会出现跨域，为了防止前端随意调用别人的后端接口

怎么解决跨域
jsonp(用的比较少,好像是通过拼凑返回内容,让前端主动调用某个申明好的js文件), cors(通过在返回头中添加header信息,比如允许的域名,允许的调用方法，允许的自定义参数)
...
https加密原理，服务端下发一份数据(感觉就是证书)，客户端上传一份数据，生成一个对称加密的密钥，然后利用这个密钥进行数据传输
```
...
单例  保证在整个应用程序的生命周期中，任何一个时刻，单例类的实例都只存在一个，同时这个类还必须提供一个访问该类的全局访问点。

工厂 定义一个创建对象的接口，但是让子类去实例化具体类。工厂方法模式让类的实例化延迟到子类中。（但是感觉和static 的延迟绑定不太一样）
...
interface 中方法的访问权限只能是public, 不能是protected或者prviate
```

```
static ,延迟静态绑定，
self, parent 调用的方法的时候都是以当前类为标准,
static 根据实际调用对象，每当到了static 的 时候调用的上一个确定了调用类的对象方法 (parent self 都不能改变调用对象)
...
就相当于我们值读入部分数据到yield 中，我们还得forea yield 抛出的数据（iterator 实现了迭代器，所以可以循环），循环去插入 取出yield 中的数据
...
iteratorAggregate 感觉就是方便的创造一个迭代器比较方便（官网上直接调用spl 的一个标准库, ArrayIterator, 数组对象转成 Iterator 方便）
```
...
innodb 中的聚簇索引的叶子结点就是数据记录或者主键，myisam中叶子结点只是数据的存储位置，所以肯定需要回表
```
...
```
mysql 中的事务有什么作用，请列举事务的隔离级别
我好像只用到了 事务的原子性（一起执行或者不执行）
```
...
利用redis的zset, 把ip地址变成数字作为score 存储，然后把各个省市的最大ip和最小ip村春，利用 zset 的范围，取到第一个最大的城市就是该城市
```
